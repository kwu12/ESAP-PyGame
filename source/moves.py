...
class Ball:
	...
	def get_rect(self):
		...
		#Rect(self.position, (self.image.get_rect().width, self.image.get_rect().height))
	...
	def update(self, deltat):
		...
		for platform in self.platforms:
			...
			if self.rect.colliderect(boundary_rect) and self.speed >= 0 and (not self.fast_falling or not platform.fall_through): #and not platform.fall_through: 
				self.speed = 0 
				self.move(x, boundary_rect.top - self.rect.height + 4) 		
				self.jump_ctr = 2	
				bools.append(True)
...
while 1:
	...
	if pygame.key.get_pressed()[pygame.K_LEFT] and ball.touching_platform:
			if(pygame.key.get_pressed()[pygame.K_UP] and ball.jump_ctr == 2):
				...
				ball.direction = "left"
				ball.state_mode = ball.RUNNING
	if pygame.key.get_pressed()[pygame.K_RIGHT] and ball.touching_platform:
			if(pygame.key.get_pressed()[pygame.K_UP] and ball.jump_ctr == 2):
				ball.jump()
				ball.anim_mode = ball.JUMPING
				ball.touching_platform = False
				...
				ball.state_mode = ball.RUNNING
	if(ball.falling or ball.jump_ctr<2):
