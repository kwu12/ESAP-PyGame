...
def game_intro(bypass = False):
	...
	if not bypass:
		...
		b4 = Button((725, 100), "../resources/GUI/Stage4_Icon.png", "Ice - 2")
...
def game_end(winner):
	...
	ball_resp = -1
	ball1_resp = -1
	ball1.move(screen.get_width() *.375, screen.get_height() * .375 - 20)
	ball.move(screen.get_width() *.625, screen.get_height() * .375 - 20)
	button_list = [b1]
	running = True
...
class Ball:
	...
	def draw(self):
		...
		if(self.falling and not(self.touching_platform) and self.anim_mode!=self.AERIAL):
	...
	def die(self):
		...
		if self.luigi:
			global ball_resp
			ball_resp = pygame.time.get_ticks() + 1500
		else:
			global ball1_resp
			ball1_resp = pygame.time.get_ticks() + 1500	
...
class Hitbox:
	...
	def update(self):
		...
		if(self.owner.direction == "left"):
			...
			self.rect = Rect(self.xPos + self.owner.position[0], self.yPos + self.owner.position[1], self.width, self.height)
		if self.owner.anim_mode == self.owner.SIDE_B:
			for proje in shot_list:
				if self.rect.colliderect(proje.rect) and proje.enemy == self.owner:
					proje.kill()
					proj = Projectile(self.owner.position, '../resources/Mario/Mario_Neutral_B/Mario_Fireball.png', self.owner.direction, self.enemy, not self.owner.luigi)
					shot_list.add(proj)
		if self.rect.colliderect(self.enemy.rect) and self.active:
			print("kb")
			hit.play()
			mod = 3 * (self.enemy.health_bar.hp / self.enemy.health_bar.size) + 1
			self.enemy.health_bar.update((self.knockbackX + self.knockbackY) / 10)
			if(self.owner.direction == "left"):
				self.enemy.add_knockback(-self.knockbackX * mod, self.knockbackY * mod, 8 * mod)
			else:
				self.enemy.add_knockback(self.knockbackX * mod, self.knockbackY * mod, 8 * mod)
			if self.owner.anim_mode == self.owner.SIDE_B:
				self.enemy.flip()
...
while 1:
	...
	ball = Ball((screen.get_width() * .625, screen.get_height() * .375 -20), hbar2, True, map1.platforms)
	ball1 = Ball((screen.get_width() *.375, screen.get_height() * .375 - 20), hbar, False, map1.platforms)
	ball.direction = "left"
	...
	while main_game:
		...
		if(ball.stunned<1): #events for player 2
			...
			if(ball.touching_platform and ball.state_mode == ball.IDLE):
				...
				if(pygame.key.get_pressed()[pygame.K_p]):
					if(ball.anim_mode!=ball.JAB):
						ball.anim_ctr = 0
						hitbox.change_rect(5,-10,35,25)
						hitbox.change_kb(8,5)
						hitbox.active = True
					ball.accx = 0
					ball.velx = 0
			...
			if pygame.key.get_pressed()[pygame.K_DOWN] and not ball.touching_platform:
				...
				pygame.time.set_timer(USEREVENT + 1, 500)
			if pygame.key.get_pressed()[pygame.K_o] and not(pygame.key.get_pressed()[pygame.K_UP] or pygame.key.get_pressed()[pygame.K_DOWN] or pygame.key.get_pressed()[pygame.K_LEFT] or pygame.key.get_pressed()[pygame.K_RIGHT]) and not(ball.anim_mode == ball.NEUTRAL_B or ball.anim_mode == ball.SIDE_B or ball.anim_mode == ball.AERIAL):
				if(ball.anim_mode!=ball.NEUTRAL_B):
					...
					shot_list.add(proj)
					shoot = True
			if pygame.key.get_pressed()[pygame.K_o] and pygame.key.get_pressed()[pygame.K_UP] and not(ball.anim_mode == ball.NEUTRAL_B or ball.anim_mode == ball.SIDE_B or ball.anim_mode == ball.AERIAL):
			...
			if(ball1.touching_platform and ball1.state_mode == ball1.IDLE):
				...
				if(pygame.key.get_pressed()[pygame.K_b]):
					if(ball1.anim_mode!=ball1.JAB):
						ball1.anim_ctr = 0
						hitbox1.change_rect(5,-10,35,25)
						hitbox1.change_kb(8,5)
						hitbox1.active = True
					ball1.accx = 0
					ball1.velx = 0
	...
	go_img = pygame.image.load("../resources/GUI/Game.png")
	screen.blit(go_img, (screen.get_width() /2 - go_img.get_width() / 2 + 30, screen.get_height() / 2- go_img.get_height() / 2, go_img.get_width(), go_img.get_height()))
	pygame.display.update()	
